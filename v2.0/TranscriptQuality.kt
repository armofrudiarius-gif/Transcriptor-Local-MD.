...
object TranscriptQuality {
    ...
    /**
     * Qualitative local estimate only. This is NOT a calibrated ASR probability.
     * It combines signal level, duration, script validity and repetition checks.
     */
    fun estimateConfidence(samples: FloatArray, globalRms: Float, text: String): ConfidenceLabel {
        if (!isUsableTranscript(text) || samples.isEmpty()) return ConfidenceLabel.LOW
        val r = rms(samples)
        val ratio = if (globalRms > 0f) r / globalRms else 0f
        val letters = text.count(Char::isLetter)
        val tokens = text.lowercase().split(Regex("\\s+")).filter { it.any(Char::isLetter) }
        val uniqueRatio = if (tokens.isEmpty()) 0f else tokens.distinct().size.toFloat() / tokens.size
        return when {
            ratio < 0.10f || letters < 3 || uniqueRatio < 0.35f -> ConfidenceLabel.LOW
            ratio > 0.35f && letters >= 10 && uniqueRatio > 0.60f -> ConfidenceLabel.HIGH
